...
const FeatureTestDashboard: React.FC = () => {
  ...
  const runAuthenticationTests = async (): Promise<TestResult[]> => {
    ...
    tests.push({
      name: 'AI API Keys',
      status: (hasGeminiKey || hasOpenAIKey) ? 'pass' : 'warning',
      message: hasGeminiKey && hasOpenAIKey ? 'Both AI API keys configured' : 
               hasGeminiKey ? 'Gemini API key configured' :
               hasOpenAIKey ? 'OpenAI API key configured' : 'No AI API keys found',
      details: `Gemini: ${hasGeminiKey ? 'Set' : 'Not set'}, OpenAI: ${hasOpenAIKey ? 'Set' : 'Not set'}`
    });
